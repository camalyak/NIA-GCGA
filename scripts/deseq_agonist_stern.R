...
# Purpose: differential gene analysis using DESeq2 - agonist: 9 mo v 18 mo
...
# sample selection: 
sm <- "XXXXXXXXXX1111111111XXXXXXXXXX0000000000"
...
gs <- factor(sml)
groups <- make.names(c("18_mo_A","9_mo_A"))
levels(gs) <- groups
...
r |>
  ...
  write.csv("data/deseq_agonist_stern.csv", row.names = FALSE)
...
dge <- read_csv("data/deseq_agonist_stern.csv")
png("images/agonist_volcano.png", width = 7, height = 6, units = "in", res = 300)
make_volcano_fc(dge, "18 mo", "Up / Downregulation in 18 mo Relative to 9 mo (Agonist)")
dev.off()
